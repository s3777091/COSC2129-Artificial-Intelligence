...
class PositionSearchProblem(search.SearchProblem):
    ...
    def getTotalNodes(self):
        """
        Returns an approximate count of the total nodes (unique Pacman positions)
        ignoring the different food configurations.
        """
        count = 0
...
class CornersProblem(search.SearchProblem):
    def __init__(self, startingGameState):
        ...
        # Check for food in corners
        for corner in self.corners:
            if not startingGameState.hasFood(*corner):
                print('Warning: no food in corner ' + str(corner))
        self._expanded = 0  # Number of search nodes

        # Initialize corners status as a dictionary
        self.cornersStatus = {corner: False for corner in self.corners}

        # Add Directions then not need create for loop again
        self.moves = {
    ...
    def getStartState(self):
        # The start state (starting position and corners status)
    ...
    def getSuccessors(self, state):
        ...
        for action, (dx, dy) in self.moves.items():
            ...
            if not self.walls[nextPos[0]][nextPos[1]]:
                cornersStatus = state[1].copy()
                if nextPos in self.corners:
                    cornersStatus[nextPos] = True

                successors.append(((nextPos, cornersStatus), action, 1))
...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    currentPosition, cornersStatus = state

    if problem.isGoalState(state):
        return 0

    unvisitedCorners = [corners[i] for i in range(len(corners)) if not cornersStatus[corners[i]]]

    if not unvisitedCorners:
        return 0

    nearestCornerDist = min(manhattanDistance(currentPosition, corner) for corner in unvisitedCorners)
    farthestCornerDist = max(manhattanDistance(currentPosition, corner) for corner in unvisitedCorners)

    return nearestCornerDist + (farthestCornerDist - nearestCornerDist)
...
def foodHeuristic(state, problem):
    ...
    # Use mazeDistance to calculate the distance to the closest food
    closestFoodDist = min(mazeDistance(position, food, problem.startingGameState) for food in foodList)

    # consider the distance to the farthest food for more accuracy
    farthestFoodDist = max(mazeDistance(position, food, problem.startingGameState) for food in foodList)

    # Combine distances for a more informed heuristic
    heuristic = closestFoodDist + (farthestFoodDist - closestFoodDist)
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        flag = True if (x, y) in Flist else False
        return flag
